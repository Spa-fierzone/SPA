...
public class AttendanceServiceImpl implements AttendanceService {
    ...
    public Attendance checkIn(Integer userId, Integer branchId, String location, String ipAddress) {
        ...
        if (!schedules.isEmpty()) {
            WorkSchedule schedule = schedules.get(0); // Get the first schedule for today
            attendance.calculateLateMinutes(schedule.getStartTime());
        }
    ...
    public Attendance checkOut(Integer userId) {
        ...
        if (!schedules.isEmpty()) {
            WorkSchedule schedule = schedules.get(0); // Get the first schedule for today
            attendance.calculateEarlyLeaveMinutes(schedule.getEndTime());

            // Calculate overtime
            LocalTime standardEndTime = schedule.getEndTime();
