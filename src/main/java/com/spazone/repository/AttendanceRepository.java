...
public interface AttendanceRepository extends JpaRepository<Attendance, Integer> {
    ...
    @Query("SELECT a FROM Attendance a WHERE a.user.userId = :userId AND MONTH(a.workDate) = :month AND YEAR(a.workDate) = :year")
    List<Attendance> findByUserAndMonthYear(@Param("userId") Integer userId,
    ...
    Long countByUserAndMonthAndYearAndStatus(@Param("userId") Integer userId,
                                             @Param("month") int month,
                                             @Param("year") int year,
    ...
    Long countLateDays(@Param("userId") Integer userId,
                       @Param("month") int month,
                       @Param("year") int year);

    @Query("SELECT COALESCE(SUM(a.totalHours), 0) FROM Attendance a WHERE a.user.userId = :userId AND MONTH(a.workDate) = :month AND YEAR(a.workDate) = :year")
    BigDecimal sumTotalHours(@Param("userId") Integer userId,
                             @Param("month") int month,
                             @Param("year") int year);

    @Query("SELECT COALESCE(SUM(a.overtimeHours), 0) FROM Attendance a WHERE a.user.userId = :userId AND MONTH(a.workDate) = :month AND YEAR(a.workDate) = :year")
    BigDecimal sumOvertimeHours(@Param("userId") Integer userId,
                                @Param("month") int month,
                                @Param("year") int year);
