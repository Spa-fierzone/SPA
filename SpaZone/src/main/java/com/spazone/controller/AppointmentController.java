...
public class AppointmentController {
    ...
    public String cancelAppointment(@PathVariable("id") Integer id,
                                    @RequestParam(name = "reason", required = false, defaultValue = "Người dùng huỷ lịch.") String reason,
                                    Authentication authentication,
                                    RedirectAttributes redirectAttributes) {
        if (!isAuthenticatedUser(authentication)) {
            return "redirect:/auth/login";
        ...
        if (appointment == null) {
            redirectAttributes.addFlashAttribute("error", "Không tìm thấy lịch hẹn hoặc bạn không có quyền huỷ.");
            return "redirect:/appointments/my";
        }

        try {
            appointmentService.cancelAppointment(id, reason);
            redirectAttributes.addFlashAttribute("success", "Huỷ lịch hẹn thành công.");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("error", "Huỷ không thành công: " + e.getMessage());
        }
